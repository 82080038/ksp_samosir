...
$(document).ready(function () {
    // Check if KSP.init exists, if not create a safe initialization
    if (typeof window.KSP.init === 'function') {
        window.KSP.init();
    } else {
        // Safe initialization for compatibility
        console.log('KSP.init not found, using safe initialization');
        if (window.KSP.setupAjaxDefaults) {
            window.KSP.setupAjaxDefaults();
        }
        if (window.KSP.setupEventListeners) {
            window.KSP.setupEventListeners();
        }
    }
});
