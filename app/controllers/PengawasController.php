...
class PengawasController extends BaseController {
    ...
    public function index() {
        ...
        $this->render(__DIR__ . '/../views/pengawas/index.php', [
            'stats' => $stats,
    ...
    public function logs() {
        ...
        $total = fetchRow("SELECT COUNT(*) as count FROM logs")['count'];
        ...
        $this->render(__DIR__ . '/../views/pengawas/logs.php', [
            'logs' => $logs,
            'page' => $page,
    ...
    public function violations() {
        ...
        $total = fetchRow("SELECT COUNT(*) as count FROM pelanggaran")['count'];
        $totalPages = ceil($total / $perPage);
        ...
        $this->render(__DIR__ . '/../views/pengawas/violations.php', [
    ...
    public function sanctions() {
        ...
        $this->render(__DIR__ . '/../views/pengawas/sanctions.php', [
            'sanctions' => $sanctions
        ]);
    }
    ...
    public function reports() {
        ...
        $total = fetchRow("SELECT COUNT(*) as count FROM laporan_pengawas")['count'];
        $totalPages = ceil($total / $perPage);
        ...
        $this->render(__DIR__ . '/../views/pengawas/reports.php', [
            'reports' => $reports,
            'page' => $page,
            'totalPages' => $totalPages
        ]);
    ...
    private function getSupervisionStats() {
        ...
        // Total violations
        $stats['total_violations'] = fetchRow("SELECT COUNT(*) as total FROM pelanggaran")['total'];

        // Pending violations
        $stats['pending_violations'] = fetchRow("SELECT COUNT(*) as total FROM pelanggaran WHERE status = 'investigasi'")['total'];

        // Total logs this month
        $stats['logs_this_month'] = fetchRow("SELECT COUNT(*) as total FROM logs WHERE MONTH(created_at) = MONTH(CURRENT_DATE) AND YEAR(created_at) = YEAR(CURRENT_DATE)")['total'];

        // Pending reports
        $stats['pending_reports'] = fetchRow("SELECT COUNT(*) as total FROM laporan_pengawas WHERE status = 'draft'")['total'];
